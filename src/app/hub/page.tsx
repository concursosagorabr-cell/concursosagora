...
import { getCachedRecentPosts, getCachedCategories } from '@/lib/sanity';
import { deduplicateCategories } from '@/utils/categories';

export const metadata: Metadata = {
  title: 'Guias de Concursos por Área — Editais, Carreiras e Notícias',
  description:
    'Nossos guias especializados organizam concursos por áreas específicas (Policiais, Tribunais, Bancários, Fiscais, Saúde, Educação e Municipais).',
...
export default async function HubsIndexPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Breadcrumb items={[{ label: 'Guias por Área' }]} />
      ...
      <header className="mb-10 bg-gradient-to-r from-slate-900 via-blue-950 to-indigo-950 text-white p-8 md:p-10 rounded-3xl shadow-xl border border-blue-900/40 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4 max-w-3xl">
          <span className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-400/30">
            🎯 Guias e Editais Especializados
          </span>
          <h1 className="text-3xl md:text-5xl font-black tracking-tight leading-tight">
            Guias de Concursos por Área & Carreiras
          </h1>
          <p className="text-blue-100/90 text-sm md:text-base leading-relaxed">
            Navegue pelos guias do portal. Nossos roteiros organizam editais, notícias e provas
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h2 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
              <span>📚</span> Todos os Guias por Área
            </h2>
            <span className="text-xs font-semibold text-slate-500">
              {CONTENT_HUBS.length} guias disponíveis
            </span>
          </div>
